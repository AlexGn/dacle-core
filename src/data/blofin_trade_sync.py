#!/usr/bin/env python3
"""
Blofin Trade Sync - Session 370

Automated trade outcome sync from Blofin exchange to close the learning loop.
Mirrors MEXC pattern from src/data/mexc_trade_sync.py for consistency.

Flow:
1. Fetch closed positions from Blofin Futures API
...
Requirements:
- BLOFIN_API_KEY, BLOFIN_API_SECRET, BLOFIN_PASSPHRASE in .env
- ccxt library (already installed)

Usage:
    from src.data.blofin_trade_sync import BlofinTradeSync
...
CLI:
    python -m src.data.blofin_trade_sync              # Sync last 7 days
    python -m src.data.blofin_trade_sync --days 30    # Sync last 30 days
    python -m src.data.blofin_trade_sync --token POWER  # Sync specific token

Author: Claude Code (Session 370)
Date: 2026-01-24
"""
...
def _generate_trade_id(trade: Dict[str, Any]) -> str:
    """Generate unique trade ID from Blofin trade data."""
...
class BlofinTradeSync:
    """
    Syncs closed trades from Blofin to DACLE's trade log.

    Automatically:
    1. Fetches closed positions from Blofin Futures
    ...
    def _init_exchange(self) -> None:
        ...
        try:
            ...
            self.exchange = ccxt.blofin({
                ...
                'options': {
                    'defaultType': 'swap',  # Futures/perpetuals trading
                }
            })
            ...
            logger.error(f"Failed to initialize Blofin: {e}")

    def is_available(self) -> bool:
        """Check if Blofin sync is available."""
    ...
    def fetch_trade_history(
        ...
        """
        Fetch trade history from Blofin.

        Args:
            since: Start date for fetching trades
            symbol: Optional specific symbol (e.g., 'BTC/USDT:USDT')

        Returns:
            List of trade dictionaries
        """
        if not self.exchange:
            logger.warning("Blofin not initialized")
            return []
    ...
    def fetch_position_history(
        ...
        try:
            ...
            # Blofin: Fetch all positions (open + recently closed)
            positions = self.exchange.fetch_positions()

            logger.info(f"Fetched {len(positions)} positions from Blofin")
            ...
            if DATA_TOKENS_DIR.exists():
                dacle_tokens = [
                    d.name for d in DATA_TOKENS_DIR.iterdir()
                ...
                for token in dacle_tokens:
                    # Convert token to Blofin symbol format (e.g., MANA -> MANA/USDT:USDT)
                    ...
                    if symbol not in seen_symbols:
                        try:
                            ...
                            # Expected for tokens not traded on Blofin or without trades
                            logger.debug(f"  {symbol}: {e}")
                            continue
    ...
    def get_open_positions(self) -> Optional[List[Dict[str, Any]]]:
        ...
        Session 370: Real-time position awareness for:
        - Dashboard display
        ...
        try:
            ...
            logger.info(f"Fetched {len(open_positions)} open positions from Blofin")
    ...
    def get_account_balance(self) -> Dict[str, Any]:
        """
        Fetch account balance and margin info from Blofin Futures.

        Session 370: Account monitoring for:
        ...
        """
        if not self.exchange:
            logger.warning("Blofin not initialized - cannot fetch balance")
            return {
                'total_equity': 0,
                'available_margin': 0,
                'used_margin': 0,
        ...
        try:
            # Fetch balance - Blofin returns different structure for futures
            balance = self.exchange.fetch_balance()

            # Blofin Futures: Look for USDT balance
    ...
    def get_full_status(self) -> Dict[str, Any]:
        """
        Get complete Blofin account status (positions + balance + metrics).

        Session 370: Combined endpoint for dashboard and L066 enforcement.
    ...
    def sync_recent_trades(
        ...
        """
        Sync recent trades from Blofin to trade_log.json.
        ...
        for blofin_trade in blofin_trades:
            try:
                ...
                # Blofin format: 'POWER/USDT:USDT' or 'POWERUSDT'
                ...
                # Generate trade ID
                trade_id = _generate_trade_id(blofin_trade)

                # Check if already synced (by Blofin order ID)
                blofin_order_id = str(blofin_trade.get('id', ''))
                if blofin_order_id in synced_ids:
                ...
                if dacle_trade:
                    ...
                    # Detect partial exits for TP scaling tracking
                    partial_exits = self._detect_partial_exits(blofin_trades, token)
                    if partial_exits:
                ...
                results["errors"].append({
                    "trade": str(blofin_trade.get('id', 'unknown')),
                    "error": str(e)
                })
    ...
    def _convert_to_dacle_format(
        ...
        try:
            ...
            # Calculate actual R:R after building trade dict
            ...
            logger.error(f"Failed to convert Blofin trade: {e}")
            return None
    ...
    def _get_playbook_rr_data(
        ...
        """
        Get R:R data from playbook if available.
    ...
    def _calculate_actual_rr(
        ...
        """
        Calculate actual R:R ratio from closed trade.
    ...
    def _detect_partial_exits(
        ...
        """
        Detect partial exits from multiple close orders.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Sync trades from Blofin")
    ...
    if args.check:
        print(f"Blofin Available: {sync.is_available()}")
        ...
        print(f"\n=== Syncing Blofin Trades (last {args.days} days, {mode}) ===\n")
