...
class OTCCommands(commands.Cog):
    ...
    @app_commands.command(name="scan-otc", description="Scan Whales Market for pre-market OTC tokens")
    ...
    async def otc_price(self, interaction: discord.Interaction, symbol: str):
        ...
        try:
            ...
            result = self.kb.client.table('otc_premarket_data') \
                .select('*') \
            ...
            if token.get('settlement_date'):
                ...
                market_field += f"**TGE:** {settlement_date.strftime('%b %d, %Y')} ({days_until} days)\n"
    ...
    @app_commands.command(name="otc-signals", description="View tokens with strong OTC trading signals")
    ...
    async def otc_signals(
        ...
        try:
            ...
            if not result.data:
                await interaction.followup.send(
                    "📭 No OTC data available.\nRun `/scan-otc` to discover tokens.",
                    ephemeral=True
                )
                return
