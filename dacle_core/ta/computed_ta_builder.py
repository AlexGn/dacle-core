...
def _compute_patterns(ohlcv: list[list]) -> list[dict]:
    ...
    try:
        from dacle_core.analysis.technical_patterns import CandlestickDetector
...
def _compute_volume_profile(ohlcv_dicts: list[dict], current_price: float = 0) -> dict:
    ...
    """
    try:
        from dacle_core.analysis.volume_profile import VolumeProfileAnalyzer
...
def _compute_emas(ohlcv: list[list]) -> dict:
    ...
    try:
        from dacle_core.ta.indicators.ema import calculate_ema
...
def _compute_chart_patterns(ohlcv: list[list], sr_levels: dict) -> list[dict]:
    ...
    """
    try:
        from dacle_core.analysis.price_action_analyzer import PatternRecognitionEngine
...
def _compute_confluence(
    ...
    try:
        from dacle_core.analysis.confluence_counter import ConfluenceCounter
...
def _compute_fibonacci(ohlcv: list[list], current_price: float) -> dict:
    ...
    try:
        from dacle_core.analysis.price_action_analyzer import PriceActionAnalyzer
...
def build_computed_ta(
    ...
    try:
        from dacle_core.analysis.momentum_indicators import (
        ...
        from dacle_core.ta.indicators.rsi import calculate_rsi as _calc_rsi
        rsi_series = []
        for i in range(len(closes)):
            if i < 15:
