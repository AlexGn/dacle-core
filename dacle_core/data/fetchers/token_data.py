...
Usage:
    from dacle_core.data.fetchers.token_data import (
...
from dacle_core.monitoring.data_source_health import get_monitor
...
def fetch_cryptorank_web(
    ...
    with _health_monitor.track_call("cryptorank_web"):
        from dacle_core.data.fetchers.cryptorank_web_fetcher import fetch_cryptorank_web as _fetch
...
def fetch_icodrops_data(
    ...
    with _health_monitor.track_call("icodrops_scraper"):
        from dacle_core.data.fetchers.icodrops_fetcher import fetch_icodrops_data as _fetch
...
def fetch_dropstab_data(
    ...
    with _health_monitor.track_call("dropstab_scraper"):
        from dacle_core.data.fetchers.dropstab_fetcher import fetch_dropstab_data as _fetch
        return _fetch(symbol, name)
...
def fetch_from_primary_sources(
    ...
    with _health_monitor.track_call("primary_source"):
        from dacle_core.data.primary_source_fetcher import fetch_from_primary_sources as _fetch
        return _fetch(
            token=token,
...
def fetch_coingecko(
    ...
    with _health_monitor.track_call("coingecko_api"):
        from dacle_core.data.primary_source_fetcher import fetch_coingecko as _fetch
...
def fetch_coinmarketcap(
    ...
    with _health_monitor.track_call("coinmarketcap_api"):
        from dacle_core.data.primary_source_fetcher import fetch_coinmarketcap as _fetch
        return _fetch(token, token_name)
